...
volatile int code,co[20],psdCheck = 0;
...
int MoveLineCheck(int check, int x, int y ,int z, int sp, int ac)
{
	...
	while(1)
	{
		if(!gtc)
		{
			...
			if(check == 2 && IR_4) {LE return 1;}
			if(check == 3 && IR_5) {LE return 2;}		
			if(check == 4 && (IR_4 || IR_5)) {LE return 2;}		
...
void MoveLine(int check, int sel ,int cm, int sp, int ac){
	...
	while(1){
		...
		if(!gtc)
		{
			...
			if(check == 3 && IR_4) {LE return;}
			if(check == 4 && IR_5) {LE return;}
...
int CPsd(int check, int ps, int cm, int v, int pm, int pc, int sp, int ac){
	...
	while(1){
		...
		if(!gtc)
		{
			...
			if(check == 13 && IR_4) {LE return 1;}
			if(check == 14 && IR_5) {LE return 1;}
...
void Avoid(int ps, int pm ,int pm2, int dis, int y, int sp, int ac){
	...
	while(1)
	{
		if(!gtc)
		{
			...
			e = s-gyro;
			e *= (sp+10)/10;
			if(fabs(e) > 30 || cnt < 30) e = 0;
		
			zig=0;
			if(y > 0){
				if(g_psd[2] < 15) psdCheck = 1;
				if(g_psd[7] < 15) psdCheck = 2;
			}
			else{
				if(g_psd[3] < 15) psdCheck = 1;
				if(g_psd[7] < 15) psdCheck = 2;
			}
...
void LineAvoid(int ps, int pm ,int pm2, int dis, int y, int sp, int ac){
	ginit("Avoid");	
...
void AB(int yp, int ym ,int xp, int xm, int zp, int zm, int xsp, int ysp)
{
	...
	xra = (double)(xm-10)/xsp;
	yra = (double)(ym-10)/ysp;
