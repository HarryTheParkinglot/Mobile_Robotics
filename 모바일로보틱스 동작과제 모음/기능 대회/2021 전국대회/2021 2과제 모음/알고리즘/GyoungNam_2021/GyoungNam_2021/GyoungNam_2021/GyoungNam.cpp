...
unsigned long puckMark[10];
///////////////////////////////////////////////////////
void Graph_Init(void);
int graph[10][10];
///////////////////////////////////////////////////////
int Slide_Puzzle_DFS(int lP, int cnt);
int dfsValidPos[2]; //한 퍽이 갈수 있는 모든 자리 (퍽추적 검사 까지 마친 걸러진 값) //fix
...
int dfsAllValidPos[10][10][2];
int dfsAllValidPosCnt[10][10];
unsigned long dfsAllMovedMap[10][10][2];
int dfsAllHeuristic[10][10][2];
...
int main()
{
	...
	{
		cin >> map[idx];
		stack[0] |= (map[idx] << (9 - idx) * 2);
	}

	for (int idx = 0; idx < 10; idx++)
	{
		cin >> aMap[idx];
		aMapB |= (aMap[idx] << (9 - idx) * 2);
	}
...
void Graph_Init(void)
{
	...
	graph[5][8] = 1; graph[8][5] = 1;

	graph[6][7] = 1; graph[7][6] = 1;
	graph[6][8] = 1; graph[8][6] = 1;
	graph[6][9] = 1; graph[9][6] = 1;

	graph[7][9] = 1; graph[9][7] = 1;
...
int Slide_Puzzle_DFS(int lP, int cnt)
{
	dfsMask = 0b11000000000000000000;
	dfsBuf = 0;

	callCnt++;

	for (int cP = 0; cP < 10; cP++) //8개의 퍽이 움직이는 모든 경우를 보기 위한 for문
	{
	...
	{
		for (int cP = 0; cP < 10; cP++) //휴리스틱 값이 같을때 탐색 순위 (크게 영향 없었던것으로 알고 있음)
		{
			for (int i = 0; i < dfsAllValidPosCnt[cnt][cP]; i++)
			{
				if (hCnt == dfsAllHeuristic[cnt][cP][i])
				{
					...
					{
						puckMark[dfsAllValidPos[cnt][cP][i]] |= (0b11000000000000000000 >> (dfsAllValidPos[cnt][cP][area] * 2));
					}

					puckMark[dfsAllValidPos[cnt][cP][i]] &= ~(0b11000000000000000000 >> (cP * 2)); //퍽 추적 데이터에서 현재 기록 삭제
...
int Search_BFS(int cP, long cMap, int* posQ) //cP에서 한번에 갈 수 있는 좌표 탐색 (posQ 배열에 저장 (포인터) )
{
	...
	while (r < f)
	{
		bfsMask = 0b11000000000000000000;

		for (int tP = 0; tP < 10; tP++)
		{
			if (tP != block && graph[bfsAPosQ[r]][tP] && !(bfsMask & cMap)) //tp가 block 이 아님 && r에서 tP가 그래프상에 연결되어 있음 && tP가 비어있음 
			{
				for (int i = 0; i < f; i++) //bfs 큐 중복검사를 위한 for문
				{
					...
					{
						if (tP != hole && !(puckMark[cP] & bfsMask)) //tp가 홀인경우 퍽을 가져다 둘 수 없음 && 퍽추적 검사 (가봤던 곳이면 안감) 
