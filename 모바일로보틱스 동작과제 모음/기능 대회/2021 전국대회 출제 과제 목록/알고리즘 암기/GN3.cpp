...
int fix[13] = {0},check[13] = {0};
int stage,flg,nowPos;
int call=  0;
int route[10],g_route[10],min;
int g_sort_route[M][2],sort_route[M][2],sort_min;
int hist[M][13];
...
void Sort(int lp, int p, int dp, int *node){
	...
	for(int i = 0 ; i < 13 && !flg ; i++){
		if(i == lp || node[i] <= 0 || fix[i] || !CheckDFS(p,i,node)) continue;
		for(int j = 0 ; j < 13 ; j++){
...
int main(){
	...
	for(stage = 0; stage <3 ; stage++){
		...
		printf("\n%d%d%d %d%d%d %d%d%d\n\n\n",map[0],map[1],map[2],map[5],map[6],map[7],map[10],map[11],map[12]);;
		printf("CALL : %d",call);
