...
void speed(double fy, double fx, double fz, double w_a){
	double ra1[3],ra2[3],v[3],da1 = 720 - w_a,da2;
	int a = 150;
	...
	for(int i = 0 ; i < 3;  i++){
		...
		SetVelocity(i,v[i]*-2.55*16777.216);
	}
	StartMotion();
	get_gyro();
}
void wrcm(double xs, double ys, double ang){
	double p[2] = {xs/100,ys/100},ane=  0;
	double g =gyro - rearGyro;
	pos[2] -= g;
	rearGyro = gyro;

	if(xs || ys){
		ane = pos[2] - ang;
		pos[0] += p[0] * cos(ane * cha) + p[1] * ((ane + 90) * cha);
		pos[1] += p[1] * cos(ane * cha) + p[0] * ((ane - 90) * cha);
	}
}
